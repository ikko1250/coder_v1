...
- ただし job 実行状態の排他制御は `App` レベルで共通化し、analysis/export/build の同時実行は許可しない。
- builder 用 runtime は `AnalysisRuntimeConfig` を流用せず、Python 実行解決だけを共有する `BuilderRuntimeConfig` を別定義する。
...
ジョブ実行中は analysis/export/build 共通の `is_any_job_running()` 判定でボタンを無効化する。
...
ただし `Default` は derive 任せにせず、Python 側既定値と一致するよう `sudachi_dict=Core`、`split_mode=C` を明示実装する。
...
`report_path` は GUI 側で常に明示指定し、Python 側の自動導出には依存しない。
...
- `current_dir` は `runtime.project_root` に固定する。
- CLI 引数は `display().to_string()` ではなく `OsString` ベースで渡す。
- stdout/stderr を回収する。
- 終了コード 0 なら `BuildCompleted(Ok(...))`
- 非 0 なら `BuildCompleted(Err(...))`

初版では stdout の JSON パースを成功条件に使わない。終了コード、stderr、既知の `report_path` を主要な結果ソースとし、stdout はログ表示用として扱う。
...
- builder を別 subprocess 経路にしても、`App` からは同じ host を通して起動させる。
...
2. 「生成した DB を現在の参照先に設定」ボタンまたは明示確認を出す
3. 利用者が切替を承認したときだけ `db_viewer_state.db_path` を更新する
4. 切替時は `db_viewer_state.reset_loaded_state()` を必ず呼ぶ
...
- `BuilderRuntimeConfig` / `AnalysisDbBuildRequest` / `BuildCompleted` 追加
- `spawn_build_job()` 追加
...
- 生成 DB を現在 DB に設定する明示導線を追加
- 反映時は `db_viewer_state.reset_loaded_state()` を呼ぶ
...
- `input_dir` が空でない
- `input_dir` が存在し、ディレクトリである
- `analysis_db_path` が空でない
- `limit` が空文字または正整数
- `purge` と `fresh_db` が同時に true でない
- `skip_tokenize == false` のとき `sudachi_dict` / `split_mode` は常に有効値

`limit` の parse は request 生成時に一度だけ行い、UI 側は入力エラー表示に専念する。builder 側でも同様の検証は行うため、GUI 側の責務は「早期フィードバック」であり、最終防衛線ではない。
...
- Windows パスと日本語パスで `OsString` 引数が崩れない
- analysis/export/build の排他制御が効く
- builder 実行中にウィンドウを閉じたとき subprocess が cleanup される
...
- build 成功時は自動切替しない
- 「生成した DB を現在 DB に設定」操作時にのみ切替える
- 切替時は loaded state をリセットする
- ただし分析実行は自動化しない
...
3. `BuilderRuntimeConfig` と共通排他制御を導入する
4. 初版から経過時間表示と close 時 cleanup を入れる
5. DB 切替は明示操作にする
...
上記指摘のうち、初版設計へ反映する判断を以下に固定する。

### 初版へ採用する

- `BuilderRuntimeConfig` を `AnalysisRuntimeConfig` から分離する
- builder でも `AnalysisProcessHost` を通す
- `App` レベルの `is_any_job_running()` で analysis/export/build を排他制御する
- GUI close 時に builder subprocess を kill する
- `--report-path` は GUI から常時指定し、Rust 側で既知にする
- builder stdout はログ扱いにし、成功判定に JSON パースを使わない
- builder subprocess の `current_dir` は `project_root` に固定する
- CLI 引数は `OsString` で構築する
- `split_mode` の既定値は Python と同じ `C` に固定する
- `input_dir` の存在確認と `purge` / `fresh_db` 排他は GUI でも事前検証する
- 初版から `DB生成中 (経過 XX 秒)` を表示する
- 成功後の DB 切替は明示操作にし、切替時に `db_viewer_state.reset_loaded_state()` を呼ぶ

### 記録のみで初版では見送る

- builder 設定の永続化
- builder stdout の構造化 progress event
- `recreate_db` の GUI 公開

### 扱いを限定する

- `line_count` と `tokens_inserted` の指摘は builder 側ロジックの品質論点として有効だが、GUI 実装の blocker ではない。builder 改修タスクとして別管理する。
- ファイル名規約違反時の warning については、現行 builder は preflight failure で停止する前提であり、「warning だけで成功継続」は現在の実装と一致しない。GUI 側では失敗時に report を開きやすくする方向で対応する。

## 実装タスク分解

以下を、反映版設計に基づく初版の正式タスクリストとする。

### A. Runtime と request 型の分離
...
1. `BuilderRuntimeConfig` を新設する
2. Python 実行解決だけを行う共通 helper を切り出す
3. `AnalysisRuntimeConfig` から builder 非依存の共通部分を整理する
4. `AnalysisDbBuildRequest` / `AnalysisDbBuildSuccess` を追加する
5. `AnalysisJobEvent::BuildCompleted` を追加する

完了条件:

- builder が `run-analysis.py` 非依存で起動設定を組み立てられる

### B. Builder subprocess 実行
...
1. `spawn_build_job()` を追加する
2. `run_build_job()` を追加する
3. `current_dir` を `project_root` に固定する
4. CLI 引数を `OsString` で組み立てる
5. `--report-path` を常時付与する
6. stdout / stderr を回収する
7. 成功判定は終了コードベースにし、stdout JSON パースを必須にしない

完了条件:

- Rust から `docs/build_ordinance_analysis_db.py` を安全に起動できる
...
1. `AnalysisProcessHost` に `spawn_build_job()` を追加する
2. `ThreadAnalysisProcessHost` から builder 実装へ委譲する

完了条件:

- `App` から analysis/export/build を同じ host 境界で起動できる
...
1. `BuilderRequestState` を追加する
2. builder 実行中状態を保持する runtime state を追加する
3. `is_any_job_running()` を追加する
4. analysis/export/build ボタンの活性判定を共通化する
5. builder 終了後に参照可能な success context を保持する

完了条件:

- analysis/export/build の同時起動が防止される
...
1. builder 設定ウィンドウを新規追加する
2. 入力フォルダー、出力 DB、report path の選択 UI を追加する
3. `skip_tokenize`、`sudachi_dict`、`split_mode`、補助フラグの UI を追加する
4. `limit` と `note` の入力 UI を追加する
5. デフォルト値を Python 側と一致させる
6. ウィンドウタイトルを分析設定と混同しない表記にする

完了条件:

- GUI から builder 用設定を一通り編集できる
...
1. `pick_open_folder()` を追加する
2. `pick_save_analysis_db()` を追加する
3. `pick_save_report_json()` を追加する

完了条件:

- builder 設定 UI に必要なパス選択が揃う
...
1. `DB生成` ボタンを追加する
2. `DB生成設定` ボタンを追加する
3. ツールバー上で analysis 群と builder 群を視覚的に分ける
4. 経過時間つき status 表示の導線を追加する

完了条件:

- builder を GUI から起動できる

### H. Builder job の開始・完了処理

対象:

- [src/app_analysis_job.rs](/mnt/f/program_2026/csv_viewer_v2/coder_v1/src/app_analysis_job.rs)
- または builder job 専用モジュール

タスク:

1. `start_build_job()` を追加する
2. `handle_build_success()` を追加する
3. `handle_build_failure()` を追加する
4. `poll_analysis_job()` で `BuildCompleted` を処理するか、builder 専用 poll を分離する
5. 失敗時に stderr と report path を利用者へ見せる
6. 成功時に「生成した DB を現在 DB に設定」導線を出す

完了条件:

- builder の成功/失敗を UI が正しく扱える

### I. DB 切替と loaded state の整合

対象:

- [src/app.rs](/mnt/f/program_2026/csv_viewer_v2/coder_v1/src/app.rs)
- [src/app_db_viewer.rs](/mnt/f/program_2026/csv_viewer_v2/coder_v1/src/app_db_viewer.rs)

タスク:

1. 明示操作でのみ `db_viewer_state.db_path` を更新する
2. 切替時に `db_viewer_state.reset_loaded_state()` を呼ぶ
3. status またはダイアログで切替結果を通知する

完了条件:

- build 後に新しい DB へ安全に切り替えられる

### J. Close 時 cleanup

対象:

- [src/app_lifecycle.rs](/mnt/f/program_2026/csv_viewer_v2/coder_v1/src/app_lifecycle.rs)
- [src/app_analysis_job.rs](/mnt/f/program_2026/csv_viewer_v2/coder_v1/src/app_analysis_job.rs)
- [src/analysis_runner.rs](/mnt/f/program_2026/csv_viewer_v2/coder_v1/src/analysis_runner.rs)

タスク:

1. builder 実行中の subprocess handle を保持する
2. close 要求時に builder subprocess を停止する
3. builder 実行中の close ガードまたは cleanup 方針を決めて実装する

完了条件:

- builder 実行中にアプリを閉じても孤立プロセスを残さない
...
1. `input_dir` 未指定を弾く
2. `input_dir` 不存在または非 directory を弾く
3. `analysis_db_path` 未指定を弾く
4. `limit` の parse を request 生成時に一度だけ行う
5. `purge` と `fresh_db` の同時指定を弾く
6. 空の `note` では `--note` を付けない

完了条件:

- 明らかな入力ミスを Python 起動前に止められる

### L. テスト

Rust 単体テスト:

1. builder CLI 引数組み立て
2. `split_mode=C` / `sudachi_dict=core` のデフォルト一致
3. `limit` parse 成功/失敗
4. `purge` と `fresh_db` の排他
5. `OsString` ベースで Windows パスを壊さない
6. event 処理に `BuildCompleted` を追加しても分岐漏れがない

GUI/統合寄りの確認:

1. analysis job 実行中は builder を起動できない
2. builder 実行中は analysis/export を起動できない
3. builder 完了後に DB 切替導線が出る
4. 切替時に loaded state がリセットされる
5. close 時に builder subprocess が cleanup される

手動確認:

1. `skip_tokenize=true` で build 成功
2. `skip_tokenize=false` で build 成功
3. 規約違反ファイルで preflight failure
4. 存在しない入力フォルダーで GUI 側エラー
5. Windows パスと日本語パスで build 成功

## 実装順の推奨

1. `BuilderRuntimeConfig` と `spawn_build_job()` を先に入れる
2. `AnalysisProcessHost` と `App` の排他制御を揃える
3. builder 設定 UI と file dialog を入れる
4. toolbar と起動導線を追加する
5. success/failure 処理と DB 切替導線を入れる
6. close 時 cleanup を入れる
7. テストと手動確認を行う

## 初版でやらないこと

- builder 設定の永続化
- JSON Lines progress event
- `recreate_db` の GUI 公開
- build 完了後の自動分析実行

必要なら次は、このタスクリストから実装順に沿って着手する。
