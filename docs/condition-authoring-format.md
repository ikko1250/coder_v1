...
- **条件ファイルの選択**：分析設定または条件エディタのファイル選択で `.json` を選べます。`format: "condition-authoring/v1"` がある JSON は authoring として認識されます。
...
- **GUI 編集**：authoring JSON は既存の条件エディタで開けません。条件エディタは runtime JSON 専用です。authoring JSON を編集する場合は、外部エディタで JSON を直接変更してください。
